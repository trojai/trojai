...
``trojai.modelgen`` is the submodule responsible for generating machine learning models from datasets and ``Experiment`` definitions created by the ``trojai.datagen`` submodule, or other datasets formatted according to what ``trojai.datagen`` would produce.  The primary classes within ``trojai.modelgen`` that are of interest are:
